...
export class ProjectsService {
  ...
  getApiProjects(): Observable<any[]> {
    const projectsURL = 'https://api.github.com/users/MacherelR/repos?sort=pushed';
    return this.http.get<any[]>(projectsURL);
  }

  getRepoInfo(name: string): Observable<any[]> {
    const reposURL = 'https://api.github.com/repos/MacherelR/' + name;
    return this.http.get<any[]>(reposURL);
  }

  getStarredProjects(): Observable<any[]> {
    const starredURL = 'https://gh-pinned-repos.now.sh/?username=MacherelR';
